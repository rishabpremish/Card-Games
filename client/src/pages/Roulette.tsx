...
type BetKind = "red" | "black" | "green" | "odd" | "even" | "1-18" | "19-36" | "1st12" | "2nd12" | "3rd12" | "number";
interface PlacedBet { kind: BetKind; amount: number; number?: number; }

const CHIP_VALUES = [1, 5, 10, 25, 100, 500];

const NUMBERS: { n: number; color: "red" | "black" | "green" }[] = [
  { n: 0, color: "green" },
  { n: 1, color: "red" }, { n: 2, color: "black" }, { n: 3, color: "red" },
  { n: 4, color: "black" }, { n: 5, color: "red" }, { n: 6, color: "black" },
  { n: 7, color: "red" }, { n: 8, color: "black" }, { n: 9, color: "red" },
  { n: 10, color: "black" }, { n: 11, color: "black" }, { n: 12, color: "red" },
  { n: 13, color: "black" }, { n: 14, color: "red" }, { n: 15, color: "black" },
  { n: 16, color: "red" }, { n: 17, color: "black" }, { n: 18, color: "red" },
  { n: 19, color: "red" }, { n: 20, color: "black" }, { n: 21, color: "red" },
  { n: 22, color: "black" }, { n: 23, color: "red" }, { n: 24, color: "black" },
  { n: 25, color: "red" }, { n: 26, color: "black" }, { n: 27, color: "red" },
  { n: 28, color: "black" }, { n: 29, color: "black" }, { n: 30, color: "red" },
  { n: 31, color: "black" }, { n: 32, color: "red" }, { n: 33, color: "black" },
  { n: 34, color: "red" }, { n: 35, color: "black" }, { n: 36, color: "red" },
];
...
function evaluateBet(bet: PlacedBet, result: number): number {
  ...
  switch (bet.kind) {
    ...
    case "green": return result === 0 ? bet.amount * 36 : 0;
...
export default function Roulette() {
  ...
  const addBet = (kind: BetKind, number?: number) => {
    ...
    const existing = bets.findIndex(b => b.kind === kind && b.number === number);
    if (existing >= 0) {
      setBets(prev => prev.map((b, i) => i === existing ? { ...b, amount: b.amount + selectedChip } : b));
    } else {
      setBets(prev => [...prev, { kind, amount: selectedChip, number }]);
    }
  };

  const clearBets = () => { if (!spinning) { setBets([]); setMessage(""); } };
    ...
    const iv = setInterval(() => {
      ...
      if (count >= maxTicks) {
        ...
        if (totalWin > 0) {
          ...
          setMessage(`${colorEmoji} ${finalResult}! Won $${totalWin.toFixed(2)} (+$${profit.toFixed(2)})`);
          setResultType("win");
          playSound("win");
          triggerConfetti({ intensity: totalWin >= totalBet * 10 ? "high" : "medium" });
          incrementWinStreak();
  ...
  return (
    <div className="rou-page">
      ...
      <style>{`
        ...
        .rou-stat { background: var(--bg-secondary); border: 3px solid var(--retro-cyan); padding: 4px 12px; box-shadow: 4px 4px 0 rgba(0,0,0,0.5); text-align: center; }
        .rou-stat-lbl { font-family: 'Press Start 2P'; font-size: clamp(0.25rem,0.7vw,0.35rem); color: var(--text-secondary); display: block; }
        .rou-stat-val { font-family: 'Press Start 2P'; font-size: clamp(0.6rem,1.8vw,1rem); color: var(--retro-green); }
        ...
        .rou-msg {
          font-family: 'Press Start 2P'; font-size: clamp(0.4rem,1.2vw,0.65rem);
        ...
        .rou-zero {
          width: 100%; padding: clamp(4px,0.8vh,8px);
        ...
        .rou-cell {
          width: clamp(24px,4.5vw,42px); height: clamp(24px,3.5vh,36px);
          display: flex; align-items: center; justify-content: center;
          font-family: 'Press Start 2P'; font-size: clamp(0.25rem,0.7vw,0.4rem);
        ...
        .rou-obtn {
          ...
          font-family: 'Press Start 2P'; font-size: clamp(0.22rem,0.6vw,0.32rem);
        ...
        .rou-hdot {
          ...
          font-family: 'Press Start 2P'; font-size: clamp(0.18rem,0.5vw,0.25rem); color: white;
        ...
        .rc {
          width: clamp(32px,5.5vw,48px); height: clamp(32px,5.5vw,48px);
          border-radius: 50%; border: 3px dashed rgba(255,255,255,0.4);
          display: flex; align-items: center; justify-content: center;
          font-family: 'Press Start 2P'; font-size: clamp(0.25rem,0.7vw,0.38rem);
        ...
        .rou-btn {
          ...
          padding: clamp(5px,0.9vh,10px) clamp(10px,2vw,20px); font-size: clamp(0.35rem,0.9vw,0.5rem);
        }
      ...
      <div className="rou-top"><h1>ROULETTE</h1></div>

      <div className="rou-stats">
        <div className="rou-stat"><span className="rou-stat-lbl">Wallet</span><span className="rou-stat-val">${(wallet ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
        <div className="rou-stat"><span className="rou-stat-lbl">Chip</span><span className="rou-stat-val">${selectedChip}</span></div>
        {totalBet > 0 && <div className="rou-stat"><span className="rou-stat-lbl">Total Bet</span><span className="rou-stat-val">${totalBet}</span></div>}
      </div>

      <div className="rou-result">
        {(spinning || result !== null) && (
          <div className={`rou-number rn-${getColor(animNumber ?? result ?? 0)} ${spinning ? "rou-spinning" : ""}`}>
            {animNumber ?? result ?? "—"}
          </div>
        )}
      ...
      <div className="rou-board-wrap">
        <div className="rou-board">
          <div className="rou-zero-row">
            <div className="rou-zero" onClick={() => addBet("number", 0)}>0</div>
          </div>
          ...
          <div className="rou-outside">
            <div className="rou-obtn ob-red" onClick={() => addBet("red")}>RED</div>
            <div className="rou-obtn ob-black" onClick={() => addBet("black")}>BLK</div>
            <div className="rou-obtn ob-green" onClick={() => addBet("green")}>0</div>
            <div className="rou-obtn" onClick={() => addBet("odd")}>ODD</div>
            <div className="rou-obtn" onClick={() => addBet("even")}>EVEN</div>
            <div className="rou-obtn" onClick={() => addBet("1-18")}>1-18</div>
            <div className="rou-obtn" onClick={() => addBet("19-36")}>19-36</div>
            <div className="rou-obtn" onClick={() => addBet("1st12")}>1st12</div>
            <div className="rou-obtn" onClick={() => addBet("2nd12")}>2nd12</div>
            <div className="rou-obtn" onClick={() => addBet("3rd12")}>3rd12</div>
          </div>
        </div>
      </div>
      ...
      {history.length > 0 && (
        <div className="rou-hist">
          {history.slice(-20).map((n, i) => (
            <div key={i} className={`rou-hdot h-${getColor(n)}`}>{n}</div>
          ))}
        </div>
      )}
      ...
      <div className="rou-controls">
        <div className="rou-chips">
          {CHIP_VALUES.map(v => (
            <div key={v} className={`rc v${v} ${selectedChip === v ? "sel" : ""}`} onClick={() => setSelectedChip(v)}>${v}</div>
          ))}
        </div>
        <div className="rou-action-row">
          <button className="rou-btn clr" onClick={clearBets} disabled={spinning}>CLEAR</button>
          <button className="rou-btn spin" onClick={spinWheel} disabled={spinning || bets.length === 0}>
